    ...
    private function handleDbauth(Request $request, $redirectRoute, $sessionValue)
    {
        ...
        if ($status == 1) {
            ...
            if ($user) {
                ...
                // Refresh this employee's identity group so declarations from
                // prior stints (rehires) and any pre-hire non-employee login
                // surface in their history. Best-effort: a linking failure must
                // never block sign-in -- the grouping is also rebuilt by the
                // identities:backfill command, and reads fall back to self.
                try {
                    app(IdentityLinkService::class)->syncFor($user);
                } catch (\Throwable $e) {
                    Log::warning('Identity link sync failed on SSO login', [
                        'user_id' => $user->id,
